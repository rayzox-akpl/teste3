...
-- Le farm est telecharge depuis GitHub a chaque lancement, avec un
-- parametre ?v=timestamp qui contourne le cache HTTP de Roblox pour
-- garantir la derniere version publiee.
...
local function launchFarm(reason)
    ...
    -- Cache-busting : le parametre ?v=<timestamp> est ignore par GitHub mais
    -- rend chaque URL unique pour Roblox, ce qui evite de recevoir une
    -- ancienne version en cache apres une mise a jour du repo.
    local url = FARM_URL .. "?v=" .. tostring(os.time())
    print("[Kronax Autoexec] telechargement du farm depuis GitHub...")

    local source
    local httpOk, httpError = pcall(function()
        -- Le second argument (true) fait renvoyer le corps de la reponse
        -- meme si le code HTTP n'est pas 200, au lieu de lever une erreur.
        source = game:HttpGet(url, true)
    end)
    if not httpOk or type(source) ~= "string" or #source == 0 then
        warn("[Kronax Autoexec] telechargement impossible : " .. tostring(httpError))
        return false
    end

    -- Le chunkname "@KronaxMinigun" apparait dans les stack traces au lieu
    -- de "[string \"...\"]", ce qui rend les messages d'erreur lisibles.
    local chunk, compileError = loadstring(source, "@KronaxMinigun")
    if not chunk then
    ...
    print("[Kronax Autoexec] farm lance : " .. tostring(reason))
    return true
end
